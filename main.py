...
def login():
    print("**************LOGIN******************")
    balance = 1000000
    account_number = input("Enter your account number: ").lower()
    password = input("Enter your password: ").lower()
    conn = sqlite3.connect("Customers_data.db")
    cur = conn.cursor()
    cur.execute("SELECT ACCOUNT_NUMBER, PASSWORD FROM Customers WHERE ACCOUNT_NUMBER = ACCOUNT_NUMBER", {'ACCOUNT_NUMBER': account_number})
